...
const (
	...
	tnsFeePerYear         = 100000000 // 1 RUNE
	txOutDelayPerYear     = 262800000000000000
	minTxOutVolumePerYear = 5256000000000000000
)

// BlocksIn returns the number of blocks in a given duration
func BlocksIn(duration time.Duration) int64 {
	return int64(duration / ThorchainBlockTime)
}

func PerBlock(perYear int64) int64 {
	blocksPerYear := BlocksIn(year)
	perBlock := perYear / blocksPerYear
	if perBlock%blocksPerYear != 0 {
		// round up
		perBlock++
	}
	return perBlock
...
// BlockTimeConstants returns constants that are derived from ThorchainBlockTime
func BlockTimeConstants() map[ConstantName]int64 {
	blocksPerYear := BlocksIn(year)

	return map[ConstantName]int64{
		BlocksPerYear:                 blocksPerYear,                   // Blocks per year
		PoolCycle:                     BlocksIn(3 * day),               // Make a pool available every 3 days
		PendingLiquidityAgeLimit:      BlocksIn(week),                  // age pending liquidity can be pending before its auto committed to the pool
		MaxAnchorBlocks:               BlocksIn(30 * time.Minute),      // max blocks to accumulate swap slips in anchor pools
		DynamicMaxAnchorSlipBlocks:    BlocksIn(2 * week),              // number of blocks to sample in calculating the dynamic max anchor slip
		DynamicMaxAnchorCalcInterval:  BlocksIn(day),                   // number of blocks to recalculate the dynamic max anchor
		FundMigrationInterval:         BlocksIn(72 * time.Minute),      // number of blocks THORNode will attempt to move funds from a retiring vault to an active one
		ChurnInterval:                 BlocksIn(3 * day),               // How many blocks THORNode try to rotate validators
		ChurnRetryInterval:            BlocksIn(72 * time.Minute),      // How many blocks until we retry a churn (only if we haven't had a successful churn in ChurnInterval blocks
		SigningTransactionPeriod:      BlocksIn(30 * time.Minute),      // how many blocks before a request to sign a tx by yggdrasil pool, is counted as delinquent.
		DoubleSignMaxAge:              BlocksIn(144 * time.Second),     // number of blocks to limit double signing a block
		FailKeygenSlashPoints:         BlocksIn(72 * time.Minute),      // slash for n blocks on keygen failure
		ObservationDelayFlexibility:   BlocksIn(2 * time.Minute),       // number of blocks of flexibility for a validator to get their slash points taken off for making an observation
		JailTimeKeygen:                BlocksIn(432 * time.Minute),     // blocks a node account is jailed for failing to keygen. DO NOT drop below tss timeout
		JailTimeKeysign:               BlocksIn(6 * time.Minute),       // blocks a node account is jailed for failing to keysign. DO NOT drop below tss timeout
		NodePauseChainBlocks:          BlocksIn(72 * time.Minute),      // number of blocks that a node can pause/resume a global chain halt
		StreamingSwapMaxLength:        BlocksIn(day),                   // max number of blocks a streaming swap can trade for
		StreamingSwapMaxLengthNative:  BlocksIn(day),                   // max number of blocks native streaming swaps can trade over
		MinTxOutVolumeThreshold:       PerBlock(minTxOutVolumePerYear), // total txout volume (in rune) a block needs to have to slow outbound transactions
		TxOutDelayRate:                PerBlock(txOutDelayPerYear),     // outbound rune per block rate for scheduled transactions (excluding native assets)
		TxOutDelayMax:                 BlocksIn(1728 * time.Minute),    // max number of blocks a transaction can be delayed
		MaxTxOutOffset:                BlocksIn(45 * time.Minute),      // max blocks to offset a txout into a future block
		ChurnOutForLowVersionBlocks:   BlocksIn(36 * time.Hour),        // the blocks after the MinJoinVersion changes before nodes can be churned out for low version
		CloutReset:                    BlocksIn(72 * time.Minute),      // number of blocks before clout spent gets reset
		RUNEPoolDepositMaturityBlocks: BlocksIn(30 * day),              // blocks from last deposit to allow withdraw
		TNSFeePerBlock:                PerBlock(tnsFeePerYear),
		TNSFeePerBlockUSD:             PerBlock(tnsFeePerYear),
	}
}
...
func NewConstantValue() *ConstantVals {
	int64Values := map[ConstantName]int64{
		...
		FailKeysignSlashPoints:              2,                  // slash for n blocks on keysign failure
		...
		EnableAdvSwapQueue:                  0,                  // enable advanced swap queue
	...
	maps.Copy(int64Values, BlockTimeConstants()) // copy block time constants into c
	return &ConstantVals{
