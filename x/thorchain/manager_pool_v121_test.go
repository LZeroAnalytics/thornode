...
type PoolMgrV121Suite struct{}

var _ = Suite(&PoolMgrV121Suite{})

func (s *PoolMgrV121Suite) TestEnableNextPool(c *C) {
	var err error
...
func (s *PoolMgrV121Suite) TestAbandonPool(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	usdAsset, err := common.NewAsset("BNB.TUSDB")
	c.Assert(err, IsNil)
...
func (s *PoolMgrV121Suite) TestDemotePoolWithLowLiquidityFees(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	usdAsset, err := common.NewAsset("BNB.TUSDB")
	c.Assert(err, IsNil)
...
func (s *PoolMgrV121Suite) TestPoolMeetTradingVolumeCriteria(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	pm := newPoolMgrV121()
...
func (s *PoolMgrV121Suite) TestRemoveAssetFromVault(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	pm := newPoolMgrV121()
...
func (s *PoolMgrV121Suite) TestRemoveLiquidityProviders(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	pm := newPoolMgrV121()
...
func (s *PoolMgrV121Suite) TestCommitPendingLiquidity(c *C) {
	var err error
