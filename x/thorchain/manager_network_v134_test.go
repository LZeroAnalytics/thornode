...
type NetworkManagerV134TestSuite struct{}

var _ = Suite(&NetworkManagerV134TestSuite{})

func (s *NetworkManagerV134TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV134TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV134(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	networkMgr := newNetworkMgrV134(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV134TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV134TestSuite) TestGetAvailablePoolsRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV134(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV134TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV134(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestRecoverPoolDeficit(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV134(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestSaverYieldFunc(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV134(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestSaverYieldCall(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	...
	networkMgr := newNetworkMgrV134(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestRagnarokPool(c *C) {
	...
	networkMgr := newNetworkMgrV134(k, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV134TestSuite) TestCleanupAsgardIndex(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (*NetworkManagerV134TestSuite) TestPOLLiquidityAdd(c *C) {
	ctx, mgr := setupManagerForTest(c)

	net := newNetworkMgrV134(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (*NetworkManagerV134TestSuite) TestPOLLiquidityWithdraw(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (*NetworkManagerV134TestSuite) TestFairMergePOLCycle(c *C) {
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV134(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV134TestSuite) TestSpawnDerivedAssets(c *C) {
	ctx, mgr := setupManagerForTest(c)

	nmgr := newNetworkMgrV134(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV134TestSuite) TestSpawnDerivedAssetsBasisPoints(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (s *NetworkManagerV134TestSuite) TestFetchMeanSlip(c *C) {
	ctx, mgr := setupManagerForTest(c)
	nmgr := newNetworkMgrV134(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
	asset := common.BTCAsset
