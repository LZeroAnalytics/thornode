...
type SwapV134Suite struct{}

var _ = Suite(&SwapV134Suite{})

func (s *SwapV134Suite) SetUpSuite(c *C) {
	err := os.Setenv("NET", "other")
	c.Assert(err, IsNil)
	SetupConfigForTest()
}

func (s *SwapV134Suite) TestSwap(c *C) {
	...
	for _, item := range inputs {
		...
		amount, evts, err := newSwapperV134().Swap(ctx, poolStorage, tx, item.target, item.destination, item.tradeTarget, "", "", nil, StreamingSwap{}, 20_000, mgr)
...
func (s *SwapV134Suite) TestSynthSwap_RuneSynthRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	pool := NewPool()
	...
	// swap rune --> synth
	{
		...
		amount, _, err := newSwapperV134().Swap(ctx, mgr.Keeper(), tx, common.ETHAsset.GetSyntheticAsset(), addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, 20_000, mgr)
		c.Assert(err, IsNil)
	...
	// swap synth --> rune
	{
		...
		amount, _, err := newSwapperV134().Swap(ctx, mgr.Keeper(), tx, common.RuneAsset(), addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, 20_000, mgr)
		c.Assert(err, IsNil)
		c.Check(amount.Uint64(), Equals, swapResult.Uint64(),
			Commentf("Actual: %d Exp: %d", amount.Uint64(), swapResult.Uint64()))
		pool, err = mgr.Keeper().GetPool(ctx, common.ETHAsset)
...
func (s *SwapV134Suite) TestSynthSwap_AssetSynth(c *C) {
	ctx, mgr := setupManagerForTest(c)
	pool := NewPool()
	pool.Asset = common.ETHAsset
	pool.BalanceRune = cosmos.NewUint(1111 * common.One)
	...
	amount, _, err = newSwapperV134().Swap(ctx, mgr.Keeper(), tx1, common.BTCAsset, addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, 20_000, mgr)
	c.Assert(err, NotNil)
...
func (s *SwapV134Suite) TestSwap_GetSwapCalc(c *C) {
	swapper := newSwapperV134()
	inputs := []struct {
		name         string
		x            cosmos.Uint
		X            cosmos.Uint
