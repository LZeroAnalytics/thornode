...
type SlashingV131Suite struct{}

var _ = Suite(&SlashingV131Suite{})

func (s *SlashingV131Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV131Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV131(k, NewDummyEventMgr())
...
func (s *SlashingV131Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV131(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV131Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV131Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV131Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV131Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV131Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV131Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV131(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV131Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV131(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV131Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV131(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV131Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
