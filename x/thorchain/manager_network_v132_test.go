...
type NetworkManagerV132TestSuite struct{}

var _ = Suite(&NetworkManagerV132TestSuite{})

func (s *NetworkManagerV132TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV132TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV132(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	networkMgr := newNetworkMgrV132(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV132TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV132TestSuite) TestGetAvailablePoolsRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV132(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV132TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV132(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestRecoverPoolDeficit(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV132(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestSaverYieldFunc(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV132(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestSaverYieldCall(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	...
	networkMgr := newNetworkMgrV132(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestRagnarokPool(c *C) {
	...
	networkMgr := newNetworkMgrV132(k, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV132TestSuite) TestCleanupAsgardIndex(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (*NetworkManagerV132TestSuite) TestPOLLiquidityAdd(c *C) {
	ctx, mgr := setupManagerForTest(c)

	net := newNetworkMgrV132(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (*NetworkManagerV132TestSuite) TestPOLLiquidityWithdraw(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (*NetworkManagerV132TestSuite) TestFairMergePOLCycle(c *C) {
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV132(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV132TestSuite) TestSpawnDerivedAssets(c *C) {
	ctx, mgr := setupManagerForTest(c)

	nmgr := newNetworkMgrV132(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV132TestSuite) TestSpawnDerivedAssetsBasisPoints(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (s *NetworkManagerV132TestSuite) TestFetchMeanSlip(c *C) {
	ctx, mgr := setupManagerForTest(c)
	nmgr := newNetworkMgrV132(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
	asset := common.BTCAsset
